...
function Strategy(options, verify) {
  options = options || {};
  options.requestTokenURL = options.requestTokenURL || 'https://api.allplayers.com/1/oauth/request_token';
  options.accessTokenURL = options.accessTokenURL || 'https://api.allplayers.com/1/oauth/access_token';
  var params = { oauth_callback: options.callbackURL };
  options.userAuthorizationURL = options.userAuthorizationURL || 'https://www.allplayers.com/1/oauth/authorize?' + querystring.stringify(params);
...
Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  this._oauth.get('https://api.allplayers.com/1/account/info', token, tokenSecret, function (err, body, res) {
